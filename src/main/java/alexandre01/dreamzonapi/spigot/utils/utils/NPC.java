...
public class NPC extends Reflections {
    ...
    public void walkToLocation(final Location location, final double speed, final EntityPlayer ep) {
        new BukkitRunnable() {
            public void run() {
                if(entityPlayer.getBukkitEntity().getLocation().distance(location) > 0.3) {
                    new BukkitRunnable() {
                        @Override
                        public void run() {
                            teleport(ep.getBukkitEntity().getLocation());
                        }
                    }.runTaskTimer(Main.getInstance(),0,1);
                    float yaw = MathUtils.getRotations(ep.getBukkitEntity().getLocation(), location)[0];
                    ...
                    if(ep.getBukkitEntity().getLocation().getY() - location.getY() > 0 && ep.getBukkitEntity().isOnGround()) {
                        direction.setY(Math.min(0.42, ep.getBukkitEntity().getLocation().getY() - location.getY()));
                    }
                    ep.getBukkitEntity().setVelocity(direction);
                } else {
                    this.cancel();
                }
            }
