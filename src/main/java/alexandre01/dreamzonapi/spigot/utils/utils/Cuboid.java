...
public class Cuboid {
        ...
        public ArrayList<Block> getBlocksBukkit(){
            ArrayList<Block> arrayList = new ArrayList<>();
            for (int i = minX; i < maxX; i++) {
                for (int j = minY; j < maxY; j++) {
                    for (int k = minZ; k < maxZ; k++) {
                        arrayList.add(getWorld().getBlockAt(i,j,k));
                    }
    ...
    public ArrayList<net.minecraft.server.v1_8_R3.Block> getBlocks(net.minecraft.server.v1_8_R3.World world){
        ArrayList<net.minecraft.server.v1_8_R3.Block> arrayList = new ArrayList<>();
        for (int i = minX; i < maxX; i++) {
            for (int j = minY; j < maxY; j++) {
                for (int k = minZ; k < maxZ; k++) {
                    //arrayList.add(world.getgetBlockAt(i,j,k));
                }
